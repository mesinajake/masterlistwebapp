 ...
 * Uses ExcelJS for robust large-file handling.
 * Handles password-protected files via officecrypto-tool decryption.
 *
 * @param buffer – raw file bytes
 * @param fileName – original filename (for validation)
 * @param password – optional password for encrypted files
 * @returns parsed headers, rows, and count
 */
export async function parseExcelBuffer(
  ...
  try {
    ...
    if (encrypted) {
      ...
      try {
        console.log("[parser] Attempting decryption...");
        const decryptedBuf = await officeCrypto.decrypt(fileBuffer, { password });
        fileBuffer = Buffer.from(decryptedBuf);
        console.log("[parser] Decryption successful, size:", fileBuffer.length, "bytes");
      } catch (decryptError) {
        const msg =
          decryptError instanceof Error
            ? decryptError.message
            : String(decryptError);
        console.error("[parser] Decryption failed:", msg);
      ...
      console.log("[parser] File is NOT encrypted — ignoring password");
    }

    // ─── Streaming Parse with ExcelJS ───────────────────
    // Uses ExcelJS.stream.xlsx.WorkbookReader to avoid loading
    // the entire decompressed XML into a single string (which
    // causes "Invalid string length" for files > ~500 MB XML).
    console.log("[parser] Reading workbook with ExcelJS streaming reader...");
    ...
    for await (const worksheetReader of workbookReader) {
      if (firstSheetProcessed) break; // only process the first sheet

      console.log("[parser] Processing sheet:", (worksheetReader as unknown as { name?: string }).name ?? "Sheet1");
      let rowIndex = 0;
      ...
      for await (const row of worksheetReader) {
        ...
        if (rowIndex === 1) {
          ...
          headersParsed = true;
          console.log("[parser] Headers:", headers.length, "columns");
          continue;
        }
        ...
        if (rows.length % 50_000 === 0) {
          console.log("[parser] ... processed", rows.length, "rows");
        }
      }
    ...
    if (rows.length === 0) {
      throw new BadRequestError(
        "Excel file is empty or contains only headers"
      );
    }

    console.log("[parser] Parsed", rows.length, "data rows total");
    ...
    console.error("[parser] Unexpected error:", msg);
    throw new BadRequestError(
      `Failed to parse Excel file: ${msg || "Unknown error"}`
    );
  }
}
